...
public class EegReader {
	...
	public EegReader(VhdrReader info) {
		this.info = info;
		channelCnt = Integer.parseInt(info.getProperties().get("CI")
				.get("NumberOfChannels"));
		if (info.getProperties().get("BI").get("BinaryFormat").equals("INT_16")) {
			binaryType = Integer.class;
		}
		if (info.getProperties().get("BI").get("BinaryFormat")
				.equals("IEEE_FLOAT_32")) {
	...
	private double[] readChannelSet(int dataPos, double[] channelSet) {
		...
		for (int i = 0; i < channelCnt; i++) {
			if (getBinarySize() == 2) {
				...
				// channelSet[i] = ((eegData[index] & 0xff) | (eegData[index+1]
				// << 8)) << 16 >> 16;
				index = index + 2;
			} else {
